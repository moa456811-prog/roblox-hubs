-- A7DEV Project Slayer 2 | Runtime V13
...
local function installCrowQuest(State,gui)
    if State.CrowQuestOps and State.CrowQuestOps.A7DEV_V13 then return end
    ...
    local Ops={A7DEV_V13=true,NextTry=0,PendingUntil=0}
...
local function installNativeSkillWatchdog(State,gui)
    if State.NativeSkillWatchdog and State.NativeSkillWatchdog.A7DEV_V13 then return end

    local Ops={A7DEV_V13=true,LastPulse=0}
    ...
    local function findRow()
        for _,row in ipairs(gui:GetDescendants()) do
            if row:IsA("TextButton") and row.Text=="" then
                local labelObj=row:FindFirstChildWhichIsA("TextLabel")
                if labelObj and low(labelObj.Text)=="auto skills" then
                    return row
                end
            end
        end
    ...
    local function click(row)
        ...
        if not ok then ok=pcall(function() row:Activate() end) end
        return ok
    end

    local function visualState(row)
        for _,child in ipairs(row:GetChildren()) do
            if child:IsA("Frame") and child.Size.X.Offset<=22 and child.Size.Y.Offset<=22 then
                local fill=child:FindFirstChildWhichIsA("Frame")
                if fill then return fill.Visible==true end
            end
        end
        return nil
    end

    local function ensureOn(forceRefresh)
        local row=findRow()
        if not row then return false end
        local current=visualState(row)
        ...
        while P.alive and gui.Parent and not State.Destroyed do
            ...
            if State.Flags.AutoSkills then
                ...
                if f.AutoFarm or f.AutoBoss or f.AutoDungeon or f.AutoYeti
                    or f.AutoHeartYeti or f.AutoFarmPlayers
                    or State.MuzanPriorityActive or State.CrowPriorityActive
                    or State.TrainingPriorityActive then
                    active=true
                end
                local target=State.CurrentTarget
                if target and fxAlive(target) then active=true end

                if active
                    and os.clock()-(State.LastSkill or 0)>4.5
                    and os.clock()-(Ops.LastPulse or 0)>5.0 then
                    Ops.LastPulse=os.clock()
                    ensureOn(true)
                end
            end
        end
