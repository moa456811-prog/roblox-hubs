...
-- =========================================================
-- V17 FINAL POLISH
-- Same validated look as V16, but event-driven and single-layer.
-- =========================================================
do
    local FINAL_BG = Color3.fromRGB(5,5,7)
    local FINAL_CONTROL = Color3.fromRGB(20,20,24)
    local FINAL_OUTLINE = Color3.fromRGB(60,60,66)
    local FINAL_TEXT = Color3.fromRGB(240,240,245)
    local FINAL_WHITE = Color3.fromRGB(245,247,250)
    local FINAL_MUTED = Color3.fromRGB(118,122,130)
    local FINAL_KNOB_OFF = Color3.fromRGB(130,131,134)

    local finalToggleGuards = setmetatable({}, {__mode="k"})
    local finalSetGuards = setmetatable({}, {__mode="k"})
    local finalLegacyGuards = setmetatable({}, {__mode="k"})

    local finalNoise = {
        "session:", "progress:", "recoveries:", "scan:", "catalog:",
        "streaming ", "selected bosses:", "boss page ", "souls idle",
        "auto sell | idle", "crafting types:", "crafting:", "fishing:",
        "god mode:", "phase=",
    }

    local function finalIsSection(o)
        return o:IsA("Frame") and string.sub(o.Name,1,8)=="Section_"
    end

    local function finalFindSection(o)
        local p=o
        while p and p~=root do
            if finalIsSection(p) then return p end
            p=p.Parent
        end
    end

    local function finalSmallControlFrame(frame)
        ...
        if parent and (parent:IsA("TextButton") or parent:IsA("TextBox")) then return true end
        return false
    end

    local function finalInteractiveContainer(frame)
        local parent=frame.Parent
        if parent and (parent:IsA("TextButton") or parent:IsA("TextBox")) then return true end
        for _,child in ipairs(frame:GetChildren()) do
            if child:IsA("TextBox") then return true end
        end
        return false
    end

    local function finalHideNoise(label)
        if not label:IsA("TextLabel") then return end
        local text=tostring(label.Text or "")
        local l=string.lower(text)
        for _,pattern in ipairs(finalNoise) do
    ...
    local function finalBlackSection(section)
        if not section or not section.Parent then return end
        section.BackgroundColor3=FINAL_BG
        section.BackgroundTransparency=0

        for _,object in ipairs(section:GetDescendants()) do
            if object:IsA("Frame") then
                if not finalSmallControlFrame(object)
                    and not finalInteractiveContainer(object)
                    and object.BackgroundTransparency<1 then
                    object.BackgroundColor3=FINAL_BG
                end
            elseif object:IsA("ScrollingFrame") then
            ...
            elseif object:IsA("TextLabel") then
                finalHideNoise(object)
            end
        end
    end

    local function finalDirectSmallBox(row)
        for _,child in ipairs(row:GetChildren()) do
            if child:IsA("Frame") and child.Name~="A7DEV_REF3_V7_SWITCH" then
                ...
                if w>0 and w<=24 and h>0 and h<=24 then return child end
            end
        end
    end

    local function finalBuildSwitch(row)
        local current=row:FindFirstChild("A7DEV_REF3_V7_SWITCH")
        if current and current:GetAttribute("A7DEV_V17")~=true then
            pcall(function() current:Destroy() end)
            current=nil
        end

        if not current then
            current=Instance.new("Frame")
            current.Name="A7DEV_REF3_V7_SWITCH"
            current:SetAttribute("A7DEV_V17",true)
            current.Position=UDim2.new(1,-38,.5,-9)
            current.Size=UDim2.fromOffset(36,18)
            current.BackgroundColor3=FINAL_CONTROL
            current.BorderSizePixel=0
            current.Active=false
            current.ZIndex=row.ZIndex+5
            current.Parent=row
            ...
            s.Name="A7DEV_V17_OUTLINE"
            s.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
            s.Color=FINAL_OUTLINE
            s.Transparency=0
            s.Thickness=1
            s.Parent=current

            local knob=Instance.new("Frame")
            knob.Name="A7DEV_V17_KNOB"
            knob.Position=UDim2.fromOffset(2,2)
            knob.Size=UDim2.fromOffset(14,14)
            knob.BackgroundColor3=FINAL_KNOB_OFF
            knob.BorderSizePixel=0
            knob.Active=false
            knob.ZIndex=current.ZIndex+1
            knob.Parent=current
    ...
    local function finalSyncToggle(label,control)
        ...
        local oldBox=finalDirectSmallBox(row)
        ...
        if labelObject then
            ...
            labelObject.TextColor3=enabled and FINAL_TEXT or FINAL_MUTED
        ...
        local track=finalBuildSwitch(row)
        track.Visible=true
        track.Position=UDim2.new(1,-38,.5,-9)
        track.Size=UDim2.fromOffset(36,18)
        track.BackgroundColor3=FINAL_CONTROL
        track.BorderSizePixel=0

        local outline=track:FindFirstChild("A7DEV_V17_OUTLINE") or track:FindFirstChildOfClass("UIStroke")
        if outline then
            outline.Color=FINAL_OUTLINE
        ...
        if knob then
            knob.Visible=true
            knob.Size=UDim2.fromOffset(14,14)
            knob.BackgroundColor3=enabled and FINAL_WHITE or FINAL_KNOB_OFF
            knob.BorderSizePixel=0
            knob.Position=UDim2.fromOffset(enabled and 20 or 2,2)
        end

        if not finalToggleGuards[row] then
            finalToggleGuards[row]=true
            on(row.Activated:Connect(function()
                task.defer(function()
                    if alive and row.Parent then finalSyncToggle(label,control) end
                end)
            end))
        end

        if type(control.Set)=="function" and not finalSetGuards[control] then
            finalSetGuards[control]=true
    ...
    local function finalSyncAllToggles()
        ...
        for label,control in pairs(controls) do
            finalSyncToggle(label,control)
        end
    end

    local function finalHideLegacy(child)
        if not child or not child.Parent or child==root or not child:IsA("GuiObject") then return end
        child.Visible=false
        if finalLegacyGuards[child] then return end
        finalLegacyGuards[child]=true
    ...
    local function finalLockLegacy()
        for _,child in ipairs(main:GetChildren()) do
            if child~=root and child:IsA("GuiObject") then finalHideLegacy(child) end
        end
    end

    local function finalApplySection(section)
        if finalIsSection(section) then
            finalBlackSection(section)
        end
    end

    -- Initial finalization spread over small batches to avoid a one-frame hitch.
    local allSections={}
    for _,object in ipairs(root:GetDescendants()) do
        if finalIsSection(object) then allSections[#allSections+1]=object end
    end

    task.spawn(function()
        for i,section in ipairs(allSections) do
            if not alive then return end
            finalApplySection(section)
    ...
    on(root.DescendantAdded:Connect(function(object)
        task.defer(function()
            if not alive or not object.Parent then return end
            local section=finalFindSection(object)
            if section then finalApplySection(section) end
            if object:IsA("TextLabel") then finalHideNoise(object) end
        end)
    end))
