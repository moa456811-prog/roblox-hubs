-- A7DEV Project Slayer 2 | Runtime V12
...
local function take(State)
    ...
    -- Try native quest transports first. Do not move the player unless they fail.
    requestMuzanQuestNative()
    if waitMuzanQuest(before,.75) then
        return true,"Muzan Quest accepted"
    end

    local now=os.clock()
    if now-(State.MuzanQuestLastNpcAction or 0)<2.5 then
        return false,"Waiting for quest confirmation"
    end
    State.MuzanQuestLastNpcAction=now
    ...
    if State.GameOps and type(State.GameOps.verifiedNpcAction)=="function" then
        ...
        if not reached then return false,tostring(msg or "Muzan unavailable") end
    end
        ...
        while P.alive and gui.Parent and not State.Destroyed do
            ...
            if State.Flags.AutoMuzanQuest then
                ...
                if not demon then
                    ...
                    if b then
                        State.MuzanQuestStatus="Waiting for "..r
                    elseif os.clock()>=nextAction then
                        nextAction=os.clock()+1.10
...
local function installCrowQuest(State,gui)
    if State.CrowQuestOps and State.CrowQuestOps.A7DEV_V12 then return end
    ...
    local Ops={A7DEV_V12=true,NextTry=0,PendingUntil=0}
...
local function installAutoSkillRecovery(State,gui)
    if State.AutoSkillRecoveryOps and State.AutoSkillRecoveryOps.A7DEV_V12 then return end
    local Ops={A7DEV_V12=true,Names={},Index=1,LastRefresh=0,Failures=0,Next=0}
    State.AutoSkillRecoveryOps=Ops
    State.Flags.AutoSkills=true

    local function refresh(force)
        if not force and os.clock()-(Ops.LastRefresh or 0)<4 then return #Ops.Names>0 end
        Ops.LastRefresh=os.clock()
        fxNative(true)

        local list={}
        local info=FXN.PlayerProfile and FXN.PlayerProfile.skill_info
        ...
        table.sort(list)
        Ops.Names=list
        Ops.Index=math.clamp(Ops.Index or 1,1,math.max(1,#list))
        return #list>0
    end

    local function combatActive()
        if not State.Flags.AutoSkills then return false end
        local f=State.Flags
        if f.AutoFarm or f.AutoBoss or f.AutoDungeon or f.AutoYeti or f.AutoHeartYeti or f.AutoFarmPlayers then return true end
        ...
        return target and fxAlive(target)
    end

    local function trySkill()
        if not combatActive() then return false end
        if os.clock()<(Ops.Next or 0) then return false end
        ...
        for _=1,#Ops.Names do
            local name=Ops.Names[Ops.Index]
            Ops.Index=Ops.Index+1
            if Ops.Index>#Ops.Names then Ops.Index=1 end

            local can=true
            if skills and type(skills.Can_Skill)=="function" then
                local ok,res=pcall(skills.Can_Skill,LocalPlayer,name)
                can=ok and res==true
            end
            ...
            if can then
                ...
                if ok and used~=false then
                    ...
                    Ops.Failures=0
                    Ops.Next=os.clock()+.48
                    if type(controller.StopHold)=="function" then
                        task.delay(.16,function()
                            pcall(controller.StopHold,name)
                        end)
                    end
                    return true
                end
        ...
        Ops.Failures=(Ops.Failures or 0)+1
        Ops.Next=os.clock()+.35
        if Ops.Failures>=3 then
            Ops.Failures=0
            FXN.Ready=false
            refresh(true)
        end
    ...
    task.spawn(function()
        while P.alive and gui.Parent and not State.Destroyed do
            task.wait(.16)
            if State.Flags.AutoSkills then
                pcall(trySkill)
            end
        end
    end)
...
local function installRestoredFeatures(State,gui)
    ...
    pcall(installAutoSkillRecovery,State,gui)
