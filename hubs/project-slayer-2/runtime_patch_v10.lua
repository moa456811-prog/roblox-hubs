...
local function take(State)
    ...
    if N.Quests and type(N.Quests.CanAddQuest)=="function" then
        ...
        if ok and can~=true then
            return false,"Muzan Quest requirements/cooldown not met"
        end
    end

    local function dialogueAdd()
        if N.Dialogue
            and type(N.Dialogue)=="table"
            and type(N.Dialogue.Functions)=="table"
            and type(N.Dialogue.Functions.AddQuest)=="function" then
            local ok,res=pcall(N.Dialogue.Functions.AddQuest,"Muzan Quest")
            if ok then return true,res end
        end
        return false
    end

    local function signalAdd()
        return send("AddQuest","Muzan Quest")
    end

    local function waitConfirm(seconds)
        local deadline=os.clock()+(seconds or 1.0)
        repeat
            if activeNow() then return true end
            task.wait(.10)
        until os.clock()>=deadline
        return activeNow()
    end

    -- First use the game's native quest helper. This can accept the quest
    -- without repeatedly moving the player back to Muzan.
    local okDialogue=dialogueAdd()
    if okDialogue and waitConfirm(.65) then
        return true,"Muzan Quest accepted via Dialogue"
    end

    if signalAdd() and waitConfirm(.65) then
        return true,"Muzan Quest accepted via AddQuest"
    end

    -- Only go to Muzan if both direct native routes were not confirmed.
    local now=os.clock()
    if now-(State.MuzanQuestLastNpcAction or 0)<2.0 then
        return false,"Waiting for Muzan Quest confirmation"
    end
    ...
    -- After the actual Muzan interaction, retry both transports and REQUIRE
    -- replicated quest state before reporting success.
    dialogueAdd()
    if waitConfirm(.75) then
        return true,"Muzan Quest accepted"
    end

    signalAdd()
    if waitConfirm(.90) then
        return true,"Muzan Quest accepted"
    end

    return false,"Muzan reached | quest not accepted yet"
end
                ...
                if not isDemon() then State.MuzanQuestStatus="Demon race required | current: "..race()
                    ...
                    elseif os.clock()>=nextAction then
                        ...
                        elseif questState()=="Doing" then
                            ...
                            State.MuzanQuestStatus=(okTake and "Active | " or "Taking quest | ")..tostring(msg)
                        end
                    end
                end
            end
...
local function installPremiumLayout(gui)
    ...
    round(ready,12); line(ready,C.line,.18)
    mk("Frame", ready, {
        Position = UDim2.fromOffset(9,8),
        Size = UDim2.fromOffset(6,6),
        BackgroundColor3 = Color3.fromRGB(55,190,145),
        BorderSizePixel = 0,
        ZIndex = 23,
    })
    mk("TextLabel", ready, {
    ...
    mk("TextLabel",community,{
        ...
        Text="Project Slayer 2 • Premium",Font=Enum.Font.Gotham,TextSize=7,TextColor3=C.muted,TextXAlignment=Enum.TextXAlignment.Left
    })
    mk("TextLabel",community,{
