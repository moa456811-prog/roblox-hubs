-- A7DEV Project Slayer 2 | Runtime V28 Farm and Boss Recovery
...
local function installMuzan(State,gui)
    ...
    local function runExact()
        if State.Runtime.specialRouteBlocked("Muzan") then
            State.MuzanPriorityActive=false
            State.MuzanQuestStatus="Waiting for current automation"
            return true
        end
...
local UI_ORGANIZATION = {
    ...
    navigation={
        FARM_CONFIG={"Farm Settings","Height, distance and safety","rbxassetid://7734053495"},
        HOME={"Home","Quick access","rbxassetid://7733960981"},
        ...
        LOOT={"Loot & Sell","Loot, selling and crafting","rbxassetid://8997386448"},
        ...
        MISC={"Config & Tools","Save config and utilities","rbxassetid://7734053495"},
    },
    sections={
...
local function installLayout(gui, State)
    ...
    local sectionSearch=mk("TextBox",header,{
        ...
        Text="",PlaceholderText="Find any feature...",ClearTextOnFocus=false,
    ...
    local home = mk("ScrollingFrame", main, {
        Position = pageHost.Position,
        Size = pageHost.Size,
        CanvasSize=UDim2.fromOffset(0,720),ScrollBarThickness=3,
        ScrollBarImageColor3=C.red,ScrollingDirection=Enum.ScrollingDirection.Y,
        BorderSizePixel=0,
        BackgroundTransparency = 1,
        Visible = false,
        ZIndex = 9,
    ...
    mk("TextLabel", hero, {
        ...
        Text = "Choose a feature below, or search by name.",
        Font = Enum.Font.Gotham,
        TextSize = 10,
        TextColor3 = C.muted,
        TextXAlignment = Enum.TextXAlignment.Left,
    ...
    local function relocateSection(section)
        ...
        if not targetInfo then
            -- Keep unlisted and newly added sections reachable in the visible column.
            for _,cols in pairs(specialColumns) do
                if cols.right and cols.right~=cols.left and section.Parent==cols.right then
                    section.Parent=cols.left
                    return true
                end
            end
            return false
    ...
    local navScroll=mk("ScrollingFrame",sidebar,{
        Position=UDim2.fromOffset(0,50),Size=UDim2.new(1,0,1,-88),
    ...
    local function navButton(key, y)
        ...
        local b = mk("TextButton", navScroll, {
    ...
    -- Flat, scrollable navigation: every destination remains visible and tappable.
    for index,key in ipairs(order) do
        local entry=navButtons[key]
        entry.button.Position=UDim2.fromOffset(8,(index-1)*38)
        entry.button.Size=UDim2.new(1,-18,0,36)
        entry.text.TextSize=12
    end
    navScroll.CanvasSize=UDim2.fromOffset(0,#order*38)
    ...
    local function homeCard(key,title,sub,x,y,w)
        ...
        mk("TextLabel",b,{
            ...
            Text=sub,Font=Enum.Font.Gotham,TextSize=11,TextColor3=C.muted,TextWrapped=true,TextXAlignment=Enum.TextXAlignment.Left,TextYAlignment=Enum.TextYAlignment.Top
        })
    ...
    local shortcuts={}
    local homeKeys={"FARM","BOSS","QUEST","LOOT","DUNGEON","FISHING","PLAYER","TELEPORT","CLAN","FARM_CONFIG","MISC"}
    for i,key in ipairs(homeKeys) do
        local meta=navMeta[key]
        shortcuts[key]=homeCard(key,meta[1],meta[2],(i-1)%2*.5,112+math.floor((i-1)/2)*100,.5)
    end

    local selectView
    local matchCounts={}
    local emptySearch=mk("TextLabel",main,{
        Position=pageHost.Position,Size=pageHost.Size,BackgroundTransparency=1,
        Text="No matching features. Try boss, loot, sell or config.",
        Font=Enum.Font.Gotham,TextSize=13,TextColor3=C.muted,
        TextWrapped=true,Visible=false,ZIndex=20,
    })
    ...
    local function filterSections(page)
        ...
        for _,section in ipairs(page:GetDescendants()) do
            if section:IsA("Frame") and string.sub(section.Name,1,8)=="Section_" then
                ...
                if section.Visible then count=count+1 end
            end
        end
        return count
    end
    ...
    local function refreshSearch()
        ...
        for _,key in ipairs(order) do
            ...
            local count=page and filterSections(page) or 0
            matchCounts[key]=count
            navButtons[key].text.Text=navMeta[key][1]..(query~="" and key~="HOME" and (" ("..count..")") or "")
            if query~="" and count>0 and not first then first=key end
        end
        if query~="" and first then
            selectView((matchCounts[selected] or 0)>0 and selected or first)
        end
        emptySearch.Visible=query~="" and not first
        if emptySearch.Visible then
            home.Visible=false
    ...
    on(sectionSearch:GetPropertyChangedSignal("Text"):Connect(function()
        ...
        task.delay(.12,function()
            if P.alive and gui.Parent and revision==searchRevision then refreshSearch() end
        end)
    end))
    selectView=function(key)
        selected=key
        emptySearch.Visible=key~="HOME" and sectionSearch.Text~="" and (matchCounts[key] or 0)==0
        sectionSearch.Visible=true
    ...
    for key,button in pairs(shortcuts) do
        on(button.Activated:Connect(function() selectView(key) end))
    end
    -- A dedicated clear action also restores filtered-out cards.
    local clearSearch=mk("TextButton",header,{
        Position=UDim2.new(1,-102,0,8),Size=UDim2.fromOffset(20,22),
        Text="x",Font=Enum.Font.GothamBold,TextSize=12,TextColor3=C.muted,
        BackgroundColor3=C.card,BorderSizePixel=0,ZIndex=26,
    })
    round(clearSearch,5)
    sectionSearch.Size=UDim2.fromOffset(152,28)
    on(clearSearch.Activated:Connect(function() sectionSearch.Text="";refreshSearch() end))
    ...
    local function queueStyle(page)
        ...
        task.defer(function()
            ...
            if sectionSearch.Text~="" then refreshSearch() end
        end)
    end
...
local function fxFindPrompt(words,maxDistance)
    ...
    local root=fxRoot()
    local best,bd
    if not P.promptIndex or os.clock()-(P.promptIndexAt or 0)>=1 then
        P.promptIndexAt=os.clock()
        P.promptIndex={}
        for _,obj in ipairs(workspace:GetDescendants()) do
            if obj:IsA("ProximityPrompt") then P.promptIndex[#P.promptIndex+1]=obj end
        end
    end
    for _,obj in ipairs(P.promptIndex) do
...
local function fxRouteBusy(State,allowPlayer)
    local f=State.Flags or {}
    if f.AutoBecomeDemon or f.AutoBecomeSlayer then return true,"Race quest" end
    if f.AutoMuzanQuest or f.AutoCrowQuest or f.AutoTrainingQuests then return true,"Special quest" end
...
local function installYeti(State,gui)
    ...
    if col and State.Runtime then
        ...
        State.Runtime.addToggle(sec,"Auto Yeti",State.Flags.AutoYeti,function(v) State.Flags.AutoYeti=v; if not v and not State.Flags.AutoHeartYeti then Ops.lastTarget=nil;Ops.lastDeath=0 end end)
        State.Runtime.addToggle(sec,"Auto Heart Yeti",State.Flags.AutoHeartYeti,function(v) State.Flags.AutoHeartYeti=v; if not v and not State.Flags.AutoYeti then Ops.lastTarget=nil;Ops.lastDeath=0 end end)
            ...
            if State.Flags.AutoYeti or State.Flags.AutoHeartYeti then
                ...
                elseif State.Flags.AutoHeartYeti and fxItemAmount("Frozen Heart")>=State.YetiHeartTarget then
                    ...
                    if Ops.lastTarget and not fxAlive(Ops.lastTarget) then
                        local hum=Ops.lastTarget:FindFirstChildOfClass("Humanoid")
                        if not hum or hum.Health>0 then
                            State.YetiStatus="Waiting for Yeti to reload"
                            continue
                        end
                    ...
                    if Ops.lastTarget and fxAlive(Ops.lastTarget) then
                        local name=target and low(target.Name) or ""
                        local minion=string.find(name,"small",1,true) or string.find(name,"mini",1,true) or string.find(name,"summon",1,true)
                        local previous=low(Ops.lastTarget.Name)
                        local previousMinion=string.find(previous,"small",1,true) or string.find(previous,"mini",1,true) or string.find(previous,"summon",1,true)
                        if not minion or previousMinion then target=Ops.lastTarget end
                    end
                    if target then
    ...
    task.spawn(function()
        while P.alive and gui.Parent do
            ...
            if State.Flags.AutoTrainingQuests then
                if State.Runtime.specialRouteBlocked("Training") then
                    State.TrainingPriorityActive=false
                    State.TrainingStatus="Waiting for current automation"
                    continue
                end
                local o=active()
                if not o then
                    ...
                    elseif string.find(n,"mizunoto",1,true) or string.find(n,"final combat",1,true) then
                        local target
                        if Ops.combatTarget and fxAlive(Ops.combatTarget) then
                            target=Ops.combatTarget
                        elseif os.clock()-(Ops.combatScanAt or -math.huge)>=1.5 then
                            Ops.combatScanAt=os.clock()
                            for _,m in ipairs(workspace:GetDescendants()) do
                                if m:IsA("Model") and fxAlive(m) and string.find(low(m.Name),"mizunoto",1,true) then target=m break end
                            end
                            Ops.combatTarget=target
                        end
...
local function installCrowQuest(State,gui)
    ...
    function Ops.tick(force)
        if State.Runtime.specialRouteBlocked("Crow") then
            State.CrowPriorityActive=false
            State.CrowQuestStatus="Waiting for current automation"
            return
        end
