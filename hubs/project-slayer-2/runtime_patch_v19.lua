-- A7DEV Project Slayer 2 | Runtime V19 Organized UI
...
local UI_ORGANIZATION = {
    order={"HOME","FARM","BOSS","DUNGEON","QUEST","LOOT","FISHING","CLAN","PLAYER","TELEPORT","MISC"},
    extraPages={"LOOT","FISHING","DUNGEON","CLAN"},
    navigation={
        HOME={"Home","Quick access","rbxassetid://7733960981"},
        FARM={"Farm","Automation, targets and positioning","rbxassetid://7733674079"},
        BOSS={"Boss","Boss selection and Yeti","rbxassetid://7733765398"},
        DUNGEON={"Dungeon","Runs, cards and souls","rbxassetid://7733917120"},
        QUEST={"Quests","Progression, race quests and training","rbxassetid://7733687281"},
        LOOT={"Inventory","Loot, selling and crafting","rbxassetid://8997386448"},
        ...
        MISC={"Settings","Configuration and utilities","rbxassetid://7734053495"},
    },
    sections={
        ["Section_Farm Automation"]={key="FARM",side="left",order=10},
        ["Section_Target Filter"]={key="FARM",side="left",order=20},
        ["Section_Farm Position"]={key="FARM",side="right",order=10},
        ["Section_Farm Safety + Session"]={key="FARM",side="right",order=20},
...
local function installLayout(gui, State)
    ...
    on(header.InputBegan:Connect(function(input)
        if minimized or input.Position.X>=minimizeButton.AbsolutePosition.X then return end
    ...
    local function relocateSection(section)
        ...
        section.LayoutOrder=targetInfo.order
    ...
    local function queueStyle(page)
        ...
        task.defer(function()
            ...
            for target in pairs(pending) do
                if target.Parent then stylePage(target) end
            end
        end)
    end
...
local function installNativeSkillWatchdog(State,gui)
    if State.NativeSkillWatchdog and State.NativeSkillWatchdog.A7DEV_V15 then return end
    local Ops={A7DEV_V15=true,LastWarmup=0}
    ...
    local function fallbackWarmup()
        fxNative(true)
        local profile=FXN.PlayerProfile
        local info=profile and profile.skill_info
        local controller=FXN.SkillController
        ...
        for name,entry in pairs(info) do
            if type(name)=="string"
                ...
                and name~="Dash"
                and name~="Double_Jump"
                and name~="Blocking" then
                names[#names+1]={Name=name,Info=entry}
            end
        end
        ...
        for _,entry in ipairs(names) do
            ...
            local meta=entry.Info
            local character=LocalPlayer.Character
            local shc=character and character:FindFirstChild("SHC")
            local before=shc and shc:GetAttribute("last_performed")
            local function confirmed()
                local c=LocalPlayer.Character
                if c~=character then return false end
                local current=c and c:FindFirstChild("SHC")
                local stamp=current and current:GetAttribute("last_performed")
                return current~=nil and current.Value==name and stamp~=nil and stamp~=before
            end

            -- First try the actual visible skill control. This reproduces the
            -- manual first-use path that initializes the game's skill controller.
            if pg then
                local wanted=low(name):gsub("[%s%p]","")
                for _,button in ipairs(pg:GetDescendants()) do
                    if button:IsA("GuiButton") and button.Visible and button.Active then
                        local text=low((button:IsA("TextButton") and button.Text or "").." "..button.Name)
                            :gsub("[%s%p]","")
                        if wanted~="" and string.find(text,wanted,1,true) then
                            local ok=false
                            if type(firesignal)=="function" then
                                ok=pcall(firesignal,button.MouseButton1Click)
                                if not ok then ok=pcall(firesignal,button.Activated) end
                            end
                            if not ok then pcall(function() button:Activate() end) end
                            task.wait(.08)
                            if confirmed() then State.LastSkill=os.clock(); return true end
                        end
                    end
                end
            end

            -- If this build exposes a bound key in skill_info, simulate the
            -- same input a manual skill press would generate.
            local key=meta.KeyCode or meta.Keybind or meta.KeyBind or meta.Key or meta.Keyboard or meta.Bind
            local enumKey
            ...
            if controller and type(controller.Attempt_Hold)=="function" then
                local ok,used=pcall(controller.Attempt_Hold,name,"A7DEV")
                if not ok then ok,used=pcall(controller.Attempt_Hold,name) end
                if ok and used==true then
                    State.LastSkill=os.clock()
                    if type(controller.StopHold)=="function" then
                        task.delay(.16,function() pcall(controller.StopHold,name) end)
                    end
                    return true
                end
            end
    ...
    local function warmup(force)
        ...
        local ex=State.A7DEVExports
        if ex and type(ex.skillWarmup)=="function" then
...
local function installRestoredFeatures(State,gui)
    ...
    if not State.A7DEVAntiIdle then
        local ok,vu=pcall(function() return game:GetService("VirtualUser") end)
        if ok and vu then
            State.A7DEVAntiIdle=on(LocalPlayer.Idled:Connect(function()
                if not P.alive or State.Destroyed then return end
